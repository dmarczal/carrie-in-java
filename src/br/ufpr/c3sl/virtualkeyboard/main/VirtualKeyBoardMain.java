...
public class VirtualKeyBoardMain extends JDialog {
	...
	public VirtualKeyBoardMain(JComponent parent, boolean enableOperations,
		...
		setAllComponetFocusAbleFalse(jpMain);
	}
	
	private void setAllComponetFocusAbleFalse(JComponent c){
		for (Component	comp : c.getComponents()) {
			comp.setFocusable(false);
			if (comp instanceof JComponent)
				setAllComponetFocusAbleFalse((JComponent)comp);
		}
	}
	...
	private void cmdNumbers(String s) {
		if (elementFocus instanceof Operation) {
			...
			newOperation();
			newNumber();
		} else if (elementFocus instanceof Variable) {
			setElementFocus(elementFocus.getFather());
		}
		...
		} else if (!elementFocus.addElement(s))
			displayErrorMessage(ERROR_MESSAGE);
	...
	private void cmdVariable(String var) {
		...
		} else if (!elementFocus.addElement(var))
			displayErrorMessage(ERROR_MESSAGE);
	...
	private boolean addElementComposite(ElementCompositeOfFormula element) {
		...
		if (elementFocus instanceof Number) {
			setElementFocus(elementFocus.getFather());
			newNumber();
		} else if (elementFocus instanceof Operation) {
			setElementFocus(elementFocus.getFather());
			newOperation();
		} else if (elementFocus instanceof Variable) {
			setElementFocus(elementFocus.getFather());
	...
	private void cmdBackspace() {
		...
		if (elementFocus.getLastElementAdded() instanceof ElementCompositeOfFormula) {
			if (elementFocus.getLastElementAdded().getLastElementAdded() instanceof ElementCompositeOfFormula)
				...
				setElementFocus(elementFocus.getLastElementAdded()
						.getLastElementAdded());
		} else if (x > 0 && elementFocus.removeLastElement()) {
...
//			
	...
	private void cmdReturn() {
		if (elementFocus.isValidElement()
				&& elementFocus instanceof ElementCompositeOfFormula) {
			setElementFocus(elementFocus.getFather());
		} else if (elementFocus.isValidElement()
	...
	private void setChangeColor(java.awt.Container c) {
		...
		for (Component comp : components) {
			...
			if (comp instanceof JComponent)
				((JComponent) comp).setBackground(new Color(220, 226, 225));
		}
	}
	...
	public void buildListOfFormula(ElementOfFormula f) {
		...
		for (Component c : f.getComponents()) {
			if (c instanceof ElementOfFormula) {
				...
				} else if (!(c instanceof FormulaInitial)) {
					listOfElementsAdded.add((ElementOfFormula) c);
					// System.out.println("formula  " + c);
				} else {
					buildListOfFormula((ElementOfFormula) c);
...
//			public void run() {
//				VirtualKeyBoardMain frame = 
//					new VirtualKeyBoardMain(new JPanel(), true, true,
//						true, true);
...
//		});
	...
	// Listener
	public synchronized void addKeyBoardListener(
	...
	public synchronized void removeKeyBoardListener(
			VirtualKeyBoardListener listener) {
		keyBoardListeners.remove(listener);
	}

	private void fireFormulaSended(JPanel jPanel) {
		VirtualKeyBoardEvent<JPanel> evento = new VirtualKeyBoardEvent<JPanel>(
				jPanel);
	...
	private List<VirtualKeyBoardListener> getListenerClone() {
		synchronized (this) {
			...
			return (List<VirtualKeyBoardListener>) ((ArrayList<VirtualKeyBoardListener>) keyBoardListeners)
					.clone();
		}
	}
